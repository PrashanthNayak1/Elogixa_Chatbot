...
const ELOGIXA_API = process.env.ELOGIXA_API_URL;

app.post("/api/contact/dialogflow-webhook", async (req, res) => {
    ...
    // Contact intent → save to MongoDB via Elogixa main server
    if (intentName.toLowerCase().includes("contact")) {
        const name = params["person"]?.name || (typeof params["person"] === "string" ? params["person"] : "") || params.name || "";
        ...
        try {
            await axios.post(`${ELOGIXA_API}/api/contact`, { name, email, country, service, message });
            return res.json({ fulfillmentText: `Thanks ${name}! We received your request and will contact you soon.` });
        } catch (err) {
            console.error("Elogixa API error:", err.message);
